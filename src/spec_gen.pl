...
generate_spec_ :-
    markdown:emit_heading(1, 'SPRIND Instruction Set Architecture Specification'),
    display_machine_overview,
    display_instructions_spec,
    true.

display_machine_overview :-
    markdown:emit_heading(2, 'Machine Overview'),
    display_gprs,
...
display_gprs :-
    markdown:emit_heading(3, 'General Purpose Registers'),
...
display_register_uses :-
    markdown:emit_heading(3, 'Register Uses and Calling Convention'),
...
display_sysregs :-
    markdown:emit_heading(3, 'System Registers'),
...
display_instructions_spec :-
    markdown:emit_heading(2, 'Instructions'),

    markdown:emit_heading(3, 'Instruction Counts by Format'),
    display_instruction_counts_by_format,

    markdown:emit_heading(3, 'Instruction Listing'),
    display_instruction_listing,

    markdown:emit_heading(3, 'Instruction Format Breakdown'),
...
display_instr_specifications :-
    markdown:emit_heading(3, 'Instruction Specifications'),
    forall(
        fmt(Fmt),
...
display_instr_specification_under_fmt(Fmt) :-
    markdown:emit_heading(4, 'Instruction Format `~k`', [Fmt]),
    forall(
...
display_instr_specification(Fmt, Instr) :-
    ...
    markdown:emit_heading(5, 'The `~w` Instruction', [Instr]),
    format('**~w** --- ~w~n', [Info.title, Info.descr]),

    ( Info.ex = [_|_] ->
        markdown:emit_heading(6, 'Examples'),
        maplist(
            [Ex]>>format('- `~w`~n', [Ex]),
            Info.ex
        )
    ...
    markdown:emit_heading(6, 'Layout'),
    ...
    markdown:emit_heading(6, 'Semantics'),
...
display_instr_format_breakdown :-
    markdown:emit_heading(4, 'Instruction Format Layouts'),
...
display_bitformat_legend :-
    markdown:emit_heading(4, 'Legend'),
